...
class RandomDeckService:
        ...
        def try_swap(
            ...
            for current in removable:
                if context.plan.primary_package in current.packages and must_package != context.plan.primary_package and not self._preserves_package_core_after_removal(context, selected, current):
                    continue
                if not self._preserves_floor_after_removal(
                    context,
                    selected,
                    current,
                    exempt_coverage_key=must_coverage_key,
                    exempt_support_axis=must_support,
                ):
                    continue
    ...
    def _preserves_package_core_after_removal(
        ...
        if context.plan.primary_package in current.packages:
            ...
            if primary_count < primary_core_floor:
                return False
            current_completion = min(1.0, float(current_completion))
            completion = min(1.0, float(completion))
            if completion + 0.01 < current_completion:
                return False
            for axis, current_state in current_axes.items():
                current_ratio = min(1.0, float(current_state.get("ratio", 0.0)))
                remaining_ratio = min(1.0, float(_axis_state.get(axis, {}).get("ratio", 0.0)))
    ...
    def _preserves_floor_after_removal(
        self,
        context: DeckContext,
        selected: Sequence[TaggedCandidate],
        current: TaggedCandidate,
        *,
        exempt_coverage_key: str | None = None,
        exempt_support_axis: str | None = None,
    ) -> bool:
        remaining = [row for row in selected if row is not current]
        remaining_coverage = self._coverage_counts(remaining)
        for key, (floor, _ceiling) in context.plan.coverage_targets.items():
            if key == exempt_coverage_key or floor <= 0:
                continue
            if current.coverage.get(key, 0.0) <= 0:
                continue
            if remaining_coverage.get(key, 0.0) + 0.01 < float(floor) * 0.95:
                return False
        remaining_support = self._support_counts(remaining, context=context)
        for axis, target in context.plan.support_targets.items():
            if axis == exempt_support_axis or target <= 0:
                continue
            if axis not in current.provides:
                continue
            if remaining_support.get(axis, 0) < int(target):
                return False
        return True
        ...
        while self._coverage_counts(selected).get(coverage_key, 0.0) < floor * 0.95:
            ...
            for current in removable:
                ...
                if not self._preserves_floor_after_removal(
                    ...
                    selected,
                    current,
                    exempt_coverage_key=coverage_key,
                ):
                    continue
